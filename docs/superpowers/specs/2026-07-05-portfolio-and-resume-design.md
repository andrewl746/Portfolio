...
3. **TeacherAId** — GenAI Genesis 2026. Firebase Auth + Firestore, misconception classification via the Anthropic Claude API called through a Cloudflare Worker proxy (keeps the API key server-side; post-hackathon revamp of the original DeepSeek/Moorcheh prototype), with a learning-memory layer for per-student history. Students answer diagnostic questions with written reasoning; system classifies misconceptions, builds per-student profiles and class-wide analytics, generates intervention/reteach suggestions and adaptive questions targeting weakest concepts.
